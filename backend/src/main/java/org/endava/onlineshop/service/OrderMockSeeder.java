...
public class OrderMockSeeder implements ApplicationRunner {
  ...
  private void seedForUserIfMissing(User user, List<SeedOrder> templates) {
        ...
        userAddresses.stream()
            .min(
                Comparator.comparing(
                    Address::getCreatedAt, Comparator.nullsLast(Comparator.naturalOrder())))
    ...
    for (int index = 0; index < templates.size(); index++) {
      ...
      order.setOrderNumber(
          template.orderNumber() + "-" + shortUserSuffix(user.getId()) + "-" + (index + 1));
      ...
      for (SeedOrderItem templateItem : template.items()) {
        Product product =
            productRepository
                .findBySlug(templateItem.productSlug())
                .orElseThrow(
                    () ->
                        new IllegalStateException(
                            "Missing product in order seed: " + templateItem.productSlug()));
        ...
        subtotal =
            subtotal.add(
                product.getBasePrice().multiply(BigDecimal.valueOf(templateItem.quantity())));
      }
  ...
  private record SeedOrderItem(String productSlug, int quantity) {}
}
